...
def main():
  ...
  parser.add_argument("--scoring", default='micro', help='f1_score.')

  numpy.random.seed(1234)
  randomState = numpy.random.RandomState(1234)
  ...
  for train_percent in training_percents:
    for shuf in shuffles:
      ...
      # X_train, y_train_ = check_X_y(X_train, y_train_[:,0], accept_sparse='csr', dtype=numpy.float64, order="C")

      label_binarizer_ = LabelBinarizer(sparse_output=True)
      Y = label_binarizer_.fit_transform(y_train_)
      Y = Y.tocsc()
      classes_ = label_binarizer_.classes_

      kFold = 10
      Cs = list(numpy.power(10.0, numpy.arange(-10, 11)))
      scores = numpy.zeros_like(Cs,  dtype=numpy.float64)
      top_k_list = [len(l) for l in y_train]
      for i, C in enumerate(Cs):
          pred_probas = numpy.zeros_like(y_train_,  dtype=numpy.float64)
          for j, column in enumerate(y_train_.T):
              # cv = check_cv(kFold, column, classifier=True)
              cv = check_cv(kFold, column)
              folds = list(cv.split(X_train, column))
              for train, val in folds:
                  clf = LogisticRegression(C = C)
                  clf.fit(X_train[train], column[train])
                  pred = clf.predict_proba(X_train[val,:])
                  pred_probas[val, j] = pred[:,1]

          # compute f1_score for aggregated predction of all k-folds
          all_labels = []
          for l, k in enumerate(top_k_list):
              probs_ = pred_probas[l, :]
              labels = classes_[probs_.argsort()[-k:]].tolist()
              all_labels.append(labels)

          scores[i] = f1_score(mlb.fit_transform(y_train), mlb.fit_transform(all_labels), average=scoring)

      # with best C, train on the full training set and test
      best_idx = scores.argmax()
      C = Cs[best_idx]
      print(C)
      clf = TopKRanker(LogisticRegression(C = C))
      clf.fit(X_train, y_train_)
      top_k_list = [len(l) for l in y_test]
      preds = clf.predict(X_test, top_k_list)

      results = {}
      results[scoring] = f1_score(mlb.fit_transform(y_test), mlb.fit_transform(preds), average=scoring)
